...
def downstream_botocore(session: nox.Session) -> None:
    ...
    session.run("python", "-c", "import urllib4; print(urllib4.__version__)")
...
def pyodideconsole(session: nox.Session) -> None:
    ...
    wheel_match = re.search(r"urllib4-[^\s]+\.whl", build_output)
    assert wheel_match
    ...
    html_content = html_content.replace("{urllib4_wheel_name}.whl", wheel_name)
