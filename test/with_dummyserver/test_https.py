...
import urllib4.http2
import urllib4.http2.probe as http2_probe
import urllib4.util as util
import urllib4.util.ssl_
...
from urllib4.connection import RECENT_DATE, HTTPSConnection, VerifiedHTTPSConnection
...
class BaseTestHTTPS(HTTPSHypercornDummyServerTestCase):
    ...
    def test_verified_with_context(self) -> None:
        ctx = util.ssl_.create_urllib4_context(
    ...
    def test_https_timeout(self) -> None:
        ...
        with HTTPSConnectionPool(
            ...
            # TODO This was removed in https://github.com/urllib4/urllib4/pull/703/files
        ...
        with HTTPSConnectionPool(
            ...
            if ssl_version is getattr(ssl, "PROTOCOL_TLS", object()):
                ...
                cmgr = pytest.warns(
                    ...
                    r"in urllib4 v2\.1\.0\. Instead use 'ssl_minimum_version'",
                )
            with cmgr:
                r = https_pool.request("GET", "/")
            assert r.status == 200, r.data
    ...
    def test_set_cert_default_cert_required(self) -> None:
        ...
        assert len(w) == 1 and str(w[0].message) == (
            "HTTPSConnection.set_cert() is deprecated and will be removed in urllib4 v2.1.0. "
    ...
    def test_set_cert_inherits_cert_reqs_from_ssl_context(
        ...
        ssl_context = urllib4.util.ssl_.create_urllib4_context(cert_reqs=verify_mode)
    ...
    def test_no_tls_version_deprecation_with_ssl_context(self) -> None:
        ...
        ctx = util.ssl_.create_urllib4_context(ssl_minimum_version=self.tls_version())
    ...
    def test_default_ssl_context_ssl_min_max_versions(self) -> None:
        ctx = urllib4.util.ssl_.create_urllib4_context()
        assert ctx.minimum_version == ssl.TLSVersion.TLSv1_2
        # urllib4 sets a default maximum version only when it is
        ...
        # https://github.com/urllib4/urllib4/issues/2477#issuecomment-1151452150
    ...
    def test_assert_missing_hashfunc(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with HTTPSConnectionPool(
            ...
            monkeypatch.setitem(urllib4.util.ssl_.HASHFUNC_MAP, digest_length, None)
            with pytest.raises(MaxRetryError) as cm:
                https_pool.request("GET", "/", retries=0)
            assert type(cm.value.reason) is SSLError
            assert (
...
class TestHTTPS_Hostname:
    ...
    def test_common_name_without_san_with_different_common_name(
        ...
        ctx = urllib4.util.ssl_.create_urllib4_context(verify_flags=0)
        try:
