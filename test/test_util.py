...
from urllib4.connection import ProxyConfig
...
from urllib4.util.proxy import connection_requires_http_tunnel
...
from urllib4.util.timeout import _DEFAULT_TIMEOUT, Timeout
from urllib4.util.url import Url, _encode_invalid_chars, parse_url
from urllib4.util.util import to_bytes, to_str
...
class TestUtil:
    ...
    def test_timeout_default_resolve(self) -> None:
        ...
        timeout = Timeout()
        with patch("urllib4.util.timeout.getdefaulttimeout", return_value=2):
            assert timeout.read_timeout == 2

        with patch("urllib4.util.timeout.getdefaulttimeout", return_value=3):
            assert timeout.read_timeout == 3
...
class TestUtilSSL:
    ...
    def test_ssl_wrap_socket_creates_new_context(
        self, create_urllib4_context: mock.MagicMock
    ) -> None:
        socket = Mock()
        ssl_wrap_socket(socket, cert_reqs=ssl.CERT_REQUIRED)

        create_urllib4_context.assert_called_once_with(None, 2, ciphers=None)
