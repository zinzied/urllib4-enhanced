...
from urllib4.exceptions import ProxySchemeUnsupported, SSLError
...
class TestSSL:
    ...
    def test_create_urllib4_context_set_ciphers(
    ...
    def test_create_urllib4_no_context(self) -> None:
        with mock.patch("urllib4.util.ssl_.SSLContext", None):
            with pytest.raises(TypeError):
                ssl_.create_urllib4_context()

    def test_create_urllib4_context_default_verify_flags(self) -> None:
    ...
    def test_create_urllib4_context_custom_verify_flags(self) -> None:
        context = ssl_.create_urllib4_context()
        assert not (context.verify_flags & ssl.VERIFY_CRL_CHECK_LEAF)
        context = ssl_.create_urllib4_context(verify_flags=ssl.VERIFY_CRL_CHECK_LEAF)
    ...
    def test_wrap_socket_no_ssltransport(self) -> None:
        with mock.patch("urllib4.util.ssl_.SSLTransport", None):
    ...
    def test_create_urllib4_context_pha(
        self,
        ...
        assert ssl_.create_urllib4_context(cert_reqs=cert_reqs) is context

        assert context.post_handshake_auth == expected_pha

    def test_create_urllib4_context_default_ciphers(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        context = mock.create_autospec(ssl_.SSLContext)
        context.set_ciphers = mock.Mock()
    ...
    def test_create_urllib4_context_ssl_version_and_ssl_min_max_version_errors(
        self, kwargs: dict[str, typing.Any]
    ) -> None:
        with pytest.raises(ValueError) as e:
            ssl_.create_urllib4_context(**kwargs)
    ...
    def test_create_urllib4_context_ssl_version_and_ssl_min_max_version_no_warning(
        self, kwargs: dict[str, typing.Any]
    ) -> None:
        ssl_.create_urllib4_context(**kwargs)
    ...
    def test_create_urllib4_context_ssl_version_and_ssl_min_max_version_no_error(
        self, kwargs: dict[str, typing.Any]
    ) -> None:
        with pytest.warns(
            DeprecationWarning,
            match=r"'ssl_version' option is deprecated and will be removed in "
            r"urllib4 v2\.1\.0\. Instead use 'ssl_minimum_version'",
        ):
            ssl_.create_urllib4_context(**kwargs)
