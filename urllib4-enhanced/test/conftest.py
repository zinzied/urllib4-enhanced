    ...
    for _ssl_version_name, min_max_version in (
        ...
        try:
            _sock = ssl_.ssl_wrap_socket(
                _sock,
                ssl_context=ssl_.create_urllib4_context(
...
def http_version(request: pytest.FixtureRequest) -> typing.Generator[str]:
    ...
    if request.param == "h2":
        ...
        from urllib4 import connection as urllib4_connection
        from urllib4.connectionpool import HTTPConnectionPool

        orig_HTTPConnection = urllib4_connection.HTTPConnection
        urllib4_connection.HTTPConnection = ErroringHTTPConnection  # type: ignore[misc,assignment]
        HTTPConnectionPool.ConnectionCls = ErroringHTTPConnection  # type: ignore[assignment]
    try:
        yield request.param
    finally:
        if request.param == "h2":
            urllib4_connection.HTTPConnection = orig_HTTPConnection  # type: ignore[misc]
