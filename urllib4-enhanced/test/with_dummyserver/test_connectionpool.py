...
from urllib4.util.timeout import _TYPE_TIMEOUT, Timeout
...
class TestConnectionPool(HypercornDummyServerTestCase):
    ...
    def test_debug_log(self, caplog: pytest.LogCaptureFixture) -> None:
        caplog.set_level(logging.DEBUG, logger="urllib4.connectionpool")
        with HTTPConnectionPool(self.host, self.port) as pool:
            r = pool.urlopen("GET", "/")
            assert r.status == 200
    ...
    def test_skip_header_non_supported(self, header: str, chunked: bool) -> None:
        with HTTPConnectionPool(self.host, self.port) as pool:
            with pytest.raises(
                ValueError,
                match="urllib4.util.SKIP_HEADER only supports 'Accept-Encoding', 'Host', 'User-Agent'",
            ) as e:
                pool.request(
    ...
    def test_request_chunked_is_deprecated(
        ...
        with HTTPConnectionPool(self.host, self.port) as pool:
            ...
            assert len(w) == 1 and str(w[0].message) == (
                "HTTPConnection.request_chunked() is deprecated and will be removed in urllib4 v2.1.0. "
...
class TestRetry(HypercornDummyServerTestCase):
    ...
    def test_default_method_forcelist_retried(self) -> None:
        """urllib4 should retry methods in the default method forcelist"""
        with HTTPConnectionPool(self.host, self.port) as pool:
            retry = Retry(total=1, status_forcelist=[418])
            resp = pool.request(
                "OPTIONS",
