...
def selenium_coverage(
    ...
    def _install_packages(self: Any) -> None:
        ...
        # install urllib4 from our test server, rather than from existing package
        result = self.run_js(
            f'await pyodide.loadPackage("http://{testserver_http.http_host}:{testserver_http.http_port}/dist/urllib4.whl")'
        )
...
_coverage= coverage.Coverage(source_pkgs=['urllib4'])
_coverage.start()
        `
        )"""
        )
