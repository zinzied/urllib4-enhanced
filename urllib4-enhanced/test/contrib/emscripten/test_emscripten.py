...
from urllib4.fields import _TYPE_FIELD_VALUE_TUPLE
...
def test_index(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int, has_jspi: bool) -> None:  # type: ignore[no-untyped-def]
        import urllib4.contrib.emscripten.fetch
        from urllib4.connection import HTTPConnection
        from urllib4.response import BaseHTTPResponse

        assert urllib4.contrib.emscripten.fetch.has_jspi() == has_jspi
...
def test_pool_requests(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int, https_port: int, has_jspi: bool) -> None:  # type: ignore[no-untyped-def]
        ...
        pool = urllib4.HTTPConnectionPool(host, port, maxsize=1, block=True)
        ...
        # data is read
        pool2 = urllib4.HTTPConnectionPool(host, port, maxsize=1, block=True)
        ...
        assert pool2.pool.qsize() == 1
        read_str = resp6.read()
        # for consistency with urllib4, this still returns the correct data even though
...
def test_timeout_warning(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        assert urllib4.contrib.emscripten.fetch._SHOWN_TIMEOUT_WARNING
...
def test_timeout_in_worker_non_streaming(
    ...
    worker_code = f"""
        from urllib4.exceptions import TimeoutError
        from urllib4.connection import HTTPConnection
...
def test_timeout_in_worker_streaming(
    ...
    worker_code = f"""
        import urllib4.contrib.emscripten.fetch
        await urllib4.contrib.emscripten.fetch.wait_for_streaming_ready()
        from urllib4.exceptions import TimeoutError
        from urllib4.connection import HTTPConnection
        conn = HTTPConnection("{testserver_http.http_host}", {testserver_http.http_port},timeout=1.0)
        result=-1
...
def test_index_https(
    ...
    @run_in_pyodide  # type: ignore[misc]
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        from urllib4.connection import HTTPSConnection
        from urllib4.response import BaseHTTPResponse
...
def test_non_streaming_no_fallback_warning(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        assert not urllib4.contrib.emscripten.fetch._SHOWN_STREAMING_WARNING
...
def test_streaming_fallback_warning(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        urllib4.contrib.emscripten.fetch.is_cross_origin_isolated = lambda: False
        ...
        assert urllib4.contrib.emscripten.fetch._SHOWN_STREAMING_WARNING
...
def test_streaming_download(
    ...
    worker_code = f"""
            ...
            response = conn.getresponse()
            assert isinstance(response, BaseHTTPResponse)
            assert urllib4.contrib.emscripten.fetch._SHOWN_STREAMING_WARNING==False
            assert(urllib4.contrib.emscripten.fetch.has_jspi() == {has_jspi})
...
def test_streaming_notready_warning(
    ...
    worker_code = f"""
        ...
        urllib4.contrib.emscripten.fetch.streaming_ready = lambda :False
        log_msgs=[]
        old_log=js.console.warn
        ...
        #assert urllib4.contrib.emscripten.fetch._SHOWN_STREAMING_WARNING==True
        """
    run_from_server.run_webworker(worker_code)
...
def test_streaming_not_ready_in_browser(
    ...
    selenium_coverage.run_async(
        """
        import urllib4.contrib.emscripten.fetch
        result=await urllib4.contrib.emscripten.fetch.wait_for_streaming_ready()
        assert(result is False)
        assert(urllib4.contrib.emscripten.fetch.streaming_ready() is None )
        """
    )
...
def test_break_worker_streaming(
    ...
    worker_code = f"""
        ...
        with pytest.raises(http.client.HTTPException):
            # make posted messages set an exception
            worker=urllib4.contrib.emscripten.fetch._fetcher.js_worker
            ...
            response = conn.getresponse()
            data=response.read()
        urllib4.contrib.emscripten.fetch._fetcher.js_worker.postMessage=old_pm
...
def test_response_init_length(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        with pytest.raises(urllib4.exceptions.InvalidHeader):
...
def test_retries(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        pool = urllib4.HTTPConnectionPool(
            host,
            port,
            maxsize=1,
            block=True,
            retries=urllib4.util.Retry(connect=5, read=5, redirect=5),
        )

        # monkeypatch connection class to count calls
        old_request = urllib4.connection.HTTPConnection.request
        count = 0
        ...
        urllib4.connection.HTTPConnection.request = count_calls  # type: ignore[method-assign]
        with pytest.raises(urllib4.exceptions.MaxRetryError):
            pool.urlopen("GET", "/")
...
def test_timeout_jspi(
    ...
    def pyodide_test(selenium, host, port):  # type: ignore[no-untyped-def]
        ...
        assert urllib4.contrib.emscripten.fetch.has_jspi() is True
        with pytest.raises(TimeoutError):
...
# (see https://github.com/urllib4/urllib4/issues/3555 )
@pytest.mark.with_jspi
...
def test_jspi_readstream_errors(
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        old_run_sync = urllib4.contrib.emscripten.fetch._run_sync_with_timeout
        with pytest.raises(TimeoutError):

            def raise_timeout(*args, **argv):  # type: ignore[no-untyped-def]
                raise urllib4.contrib.emscripten.fetch._TimeoutError()

            urllib4.contrib.emscripten.fetch._run_sync_with_timeout = raise_timeout
            response.read()
        urllib4.contrib.emscripten.fetch._run_sync_with_timeout = old_run_sync
        conn.request("GET", url, preload_content=False)
        response = conn.getresponse()
        with pytest.raises(HTTPException):

            def raise_error(*args, **argv):  # type: ignore[no-untyped-def]
                raise urllib4.contrib.emscripten.fetch._RequestError()

            urllib4.contrib.emscripten.fetch._run_sync_with_timeout = raise_error
            response.read()
    ...
    def pyodide_test(selenium_coverage, host: str, port: int) -> None:  # type: ignore[no-untyped-def]
        ...
        if hasattr(pyodide.ffi, "can_run_sync"):
            ...
            def should_return_false(func):  # type: ignore[no-untyped-def]
                ...
                from urllib4.contrib.emscripten.fetch import has_jspi
