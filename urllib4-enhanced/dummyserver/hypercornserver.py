...
class Config(hypercorn.Config):
    def create_sockets(self) -> hypercorn.config.Sockets:
        ...
        if self.ssl_enabled:
            secure_sockets = self._create_urllib4_sockets(self.bind[0])
        else:
            insecure_sockets = self._create_urllib4_sockets(self.bind[0])
        return hypercorn.config.Sockets(
            secure_sockets, insecure_sockets, quic_sockets=[]
        )

    def _retry_create_urllib4_sockets(self, bind: str) -> list[socket.socket]:
        ...
        # waste about 2 second for each test on Windows since urllib4 tries
        ...
        # _create_urllib4_sockets up to 10 times, which completely eliminates
        # this failure mode.
        for i in range(10):
            try:
                return self._create_urllib4_sockets(bind)
            except OSError as e:
    ...
    def _create_urllib4_sockets(self, bind: str) -> list[socket.socket]:
        sockets = []
